...
class TaskManager:
    ...
    def update_status(self, task_id: int, new_status: Status) -> bool:
        for t in self.tasks:
            if t.id == task_id:
                t.status = new_status
                return True
...
def main() -> None:
    ...
    while True:
        print("\nMenu:")
        print("  1) Adicionar tarefa (modelo acadêmico)")
        print("  2) Adicionar tarefa (custom)")
        ...
        if choice == "1":
            ...
            task_id = read_int("Digite o ID da tarefa para remover: ")
            ok = manager.remove_task(task_id)
            print("Removida." if ok else "ID não encontrado.")

        elif choice == "5":
            task_id = read_int("Digite o ID da tarefa: ")
            new_status = choose_status()
            ok = manager.update_status(task_id, new_status)
            print("Status atualizado." if ok else "ID não encontrado.")
